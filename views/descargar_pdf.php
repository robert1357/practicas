...
if (strpos($doc_id, 'plan_') === 0) {
    $plan_id = str_replace('plan_', '', $doc_id);
    $planModel = new PlanPractica();
    $plan = $planModel->findById($plan_id);
    
    if (!$plan || $plan['estudiante_id'] != $user['id']) {
        http_response_code(403);
        die('No tienes permisos para descargar este documento');
    }
} elseif (strpos($doc_id, 'informe_') === 0) {
    $informe_id = str_replace('informe_', '', $doc_id);
    $informeModel = new InformeFinal();
    $informe = $informeModel->findById($informe_id);
    
    if (!$informe || $informe['estudiante_id'] != $user['id']) {
        http_response_code(403);
        die('No tienes permisos para descargar este documento');
    }
}
...
if (!file_exists($full_path)) {
    ...
    die('Archivo no encontrado: ' . $full_path);
}
