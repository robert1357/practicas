...
// Obtener docentes para el jurado
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $data = [
        ...
        'presidente_jurado' => $_POST['presidente_jurado'],
        'vocal_jurado' => $_POST['vocal_jurado'],
        'secretario_jurado' => $_POST['secretario_jurado'],
...
<body class="bg-gray-50 min-h-screen">
                                        ...
                                        <div class="bg-white p-3 rounded-lg border border-gray-200">
                                            <div class="flex justify-between items-start mb-2">
                                                ...
                                                <span class="px-2 py-1 rounded text-xs font-medium bg-blue-100 text-blue-800">
                                                    <?php echo ucfirst($sustentacion['estado']); ?>
                                                </span>
                                            </div>
                                            <p class="text-sm text-gray-600">
                                                <i class="fas fa-clock mr-1"></i>
                                                <?php echo $sustentacion['hora_sustentacion']; ?>
                                            </p>
                                            <p class="text-sm text-gray-600">
                                                <i class="fas fa-map-marker-alt mr-1"></i>
                                                <?php echo htmlspecialchars($sustentacion['lugar']); ?>
                                            </p>
                                        </div>
                                    <?php endforeach; ?>
                                <?php endif; ?>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const juradoSelects = ['presidente_jurado', 'vocal_jurado', 'secretario_jurado'];
            
            juradoSelects.forEach(selectName => {
                document.querySelector(`select[name="${selectName}"]`).addEventListener('change', function() {
                    const selectedValue = this.value;
                    const otherSelects = juradoSelects.filter(name => name !== selectName);
                    
                    // Validar que no se repita la selección
                    otherSelects.forEach(otherSelectName => {
                        const otherSelect = document.querySelector(`select[name="${otherSelectName}"]`);
                        if (otherSelect.value === selectedValue && selectedValue !== '') {
                            alert('No se puede seleccionar el mismo docente para diferentes roles en el jurado');
                            this.value = '';
                        }
                    });
                });
